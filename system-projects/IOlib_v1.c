...
/* Упражнения 8.2 - 8.4 из книги K&R 2-е издание, версия с явными битовыми операциями */
...
int f_flush(file *fp)
{
    ...
    if (fp == NULL) {
        ...
        if ((fp->flag & _WRITE) == 0) /* Проверяем, что открыт на запись */
            return EOF;
...
int f_seek(file *fp, long offset, int origin)
{
    ...
    /* Сбрасываем флаги */
    fp->cnt = 0;
    fp->flag &= ~(_EOF); /* Сбрасываем флаг конца файла */
    return 0;
}
